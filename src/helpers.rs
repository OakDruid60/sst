...
pub fn compact_summary_string(qi_vec: &Vec<SectorInfo>) -> String {
    let stats = crate::statistics::calculate(qi_vec);
...
///
pub fn alert_status_of_quadrant(qi_vec: &Vec<SectorInfo>) -> String {
    let cur_alert = calc_alert_status(qi_vec);
    let mut stat_string = "Normal".on_black().to_string();
    if cur_alert == AlertStatus::Docked {
        stat_string = "DOCKED".bright_cyan().to_string();
        return stat_string;
    }
    'outer: for si in qi_vec.iter() {
        let n_info = *si;
        if n_info.get_sector_type() == SectorType::Klingon {
            stat_string = "RED".bold().on_red().to_string();
            break 'outer;
        } else if n_info.get_sector_type() == SectorType::Romulan {
            stat_string = "YELLOW".bold().on_yellow().to_string();
        }
    }
    return stat_string;
}
...
/// # calc_alert_status
///
pub fn calc_alert_status(qi_vec: &Vec<SectorInfo>) -> AlertStatus {
    let mut cur_alert = AlertStatus::Normal;
    // find the enterprise
    let enterprise_vec = crate::gamedata::create_vec_of_type(qi_vec, SectorType::Enterprise);
    let star_base_vec = crate::gamedata::create_vec_of_type(qi_vec, SectorType::Starbase);
    if star_base_vec.len() != 0 {
        //println!("{:?}", enterprise_vec[0]);
        //println!("{:?}", star_base_vec[0]);
        let distance = crate::location::calc_sector_distance(
            enterprise_vec[0].get_location(),
            star_base_vec[0].get_location(),
        );
        //println!("{}",distance);
        if distance < 1.42 {
